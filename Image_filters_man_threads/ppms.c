...
void filterImage(ppm_t * source, char* filter_file)
{
    ...
    //Get number of cores
    long ncores = getNumberCores();
    if (ncores>0)
        NUM_THREADS = ncores;
    //Storing data needed in thread
    thread_t thread_data;
    thread_data.center_point_kernel = center_point_kernel;
    thread_data.dest = &dest_image;
    ...
    {
        thread_data.start = i; //Start point of each thread
        printf("%d\n", thread_data.start);
        pthread_create(&tid[i], NULL, &threadFilter, (void*)&thread_data);
    }
...
{
    thread_t *data = arg; //Stores all data needed by thread
    float totalr = 0;
    float totalg = 0;
    float totalb = 0;

    //Parallelization and filtering of image
    for (int i=data->center_point_kernel+data->start;i<data->source->height-data->center_point_kernel;i=i+NUM_THREADS)
    {
        for (int j=data->center_point_kernel;j<data->source->width-data->center_point_kernel;j++)
        {
            totalr=0;
            totalb=0;
            totalg=0;
            for (int a=0-data->center_point_kernel;a<=data->center_point_kernel;a++)
            {
                for (int b=0-data->center_point_kernel;b<=data->center_point_kernel;b++)
                {
                    //Get the summatory of the product between the image and the convolution matrix
                    totalr += data->kernel->matrix[a+data->center_point_kernel][b+data->center_point_kernel] * data->source->pixels[i+a][j+b].data[0];
                    totalg += data->kernel->matrix[a+data->center_point_kernel][b+data->center_point_kernel] * data->source->pixels[i+a][j+b].data[1];
                    totalb += data->kernel->matrix[a+data->center_point_kernel][b+data->center_point_kernel] * data->source->pixels[i+a][j+b].data[2];
                }
            }
            //Apply results to filtered image
            data->dest->pixels[i][j].data[0] = (unsigned char) totalr;
            data->dest->pixels[i][j].data[1] = (unsigned char) totalg;
            data->dest->pixels[i][j].data[2] = (unsigned char) totalb;
        }
    }
